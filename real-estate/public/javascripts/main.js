$(document).ready(() => {
    ...
    //         .then(data => {
    //             data.forEach(value => {
    //                 $(".city-picker").append(`<option value="${value.cityId}">${value.name}</option>`)
    //             })
    ...
    $('body').on('click', ".edit-property-btn", (e)=>{
        ...
        window.location.href = window.location.origin+"/properties/edit-property/"+propertyId
    })

    $(".confirm-yes").on('click', (e) => {
        var propertyId = e.target.dataset.id
        fetch(`/properties/${propertyId}`,{method:"DELETE"}).then(data=>data.json())
        .then(data=>{
            if (data.code===0){
                var path = window.location.pathname
                if (path.includes("/properties/") && !isNaN(path.replace("/properties/",""))){
                    var page = Number(path.replace("/properties/",""))
                    searchData(page)
...
var swiper = new Swiper('.swiper-container', {
  ...
  const loadDistrict = (selectedCity, selectedDistrict) => {
      ...
      if (selectedCity === "any" || !selectedCity) {
        if (window.location.pathname === "/properties/add-property" || window.location.pathname.includes("/properties/edit-property/")) {
  ...
  const searchData = (page) =>{
    ...
    var formData = {noItem, sortBy, keyword, type, city, district, isSale, areaFrom, areaTo, priceFrom, priceTo, bedrooms, rooms, floors, bathrooms, userId}
    ...
    fetch(`/properties/${page}`, {method: "POST",headers:{'Content-Type': 'application/json'},  body:JSON.stringify(formData)}).then(data=>data.json())
    .then(result=>{
        container.find("li").remove()
